...
export function generateOccurrencesForSchedule(schedule, windowStart: string, windowEnd: string) {
  ...
  if (schedule.recurrenceType === RecurrenceType.WEEKLY) { 
    ...
    for (let dt = new Date(current); dt <= rangeEnd; dt = addDays(dt, schedule.interval || 1)) {
      if (endDate && isAfter(dt, endDate)) break;
      if (dt >= start) results.push(dt.toISOString().slice(0, 10));
    }
  } else if (schedule.recurrenceType === RecurrenceType.MONTHLY) {
    for (let dt = new Date(current); dt <= rangeEnd; dt = addMonths(dt, schedule.interval || 1)) {
      if (endDate && isAfter(dt, endDate)) break;
      if (dt >= start) results.push(dt.toISOString().slice(0, 10));
    }
  } else { 
