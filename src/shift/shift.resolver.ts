...
export class ShiftResolver {
    ...
    @Mutation(() => Boolean)
    async addSchedule(@Args('shiftId') shiftId: string, @Args('input') input: CreateScheduleInput) {
    ...
    async generatedOccurrences(
        ...
        return generated.map(g => { return { date: g.date, name: g.shift.name, startTime: g.shift.startTime } });
    }
}
