...
export class ShiftService {
    ...
    constructor(
        ...
        @InjectRepository(ShiftSchedule) private scheduleRepo: Repository<ShiftSchedule>,
    ...
    async addSchedule(shiftId: string, scheduleData) {
        ...
        const schedule = this.scheduleRepo.create({ ...scheduleData, shift: shift });
    ...
    async ensureOccurrence(shiftTemplateId: string, date: string) {
        let occ = await this.occRepo.findOne({ where: { shiftTemplate: { id: shiftTemplateId }, date } });
        if (!occ) {
